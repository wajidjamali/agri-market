import React from "react";
import { AiOutlineFolderAdd, AiOutlineGift } from "react-icons/ai";
import { FiPackage, FiShoppingBag } from "react-icons/fi";
import { MdOutlineLocalOffer } from "react-icons/md";
import { RxDashboard } from "react-icons/rx";
import { VscNewFile } from "react-icons/vsc";
import { CiMoneyBill, CiSettings } from "react-icons/ci";
import { Link } from "react-router-dom";
import { BiMessageSquareDetail } from "react-icons/bi";
import { HiOutlineReceiptRefund } from "react-icons/hi";

const DashboardSideBar = ({ active }) => {
  return (
    <div className='w-full h-full bg-white shadow-sm overflow-y rounded-md sticky top-0 left-0 z-10'>
      {/* single item */}
      <div className='w-full flex items-center p-3'>
        <Link to='/dashboard' className='w-full flex items-center'>
          <RxDashboard
            size={28}
            color={`${active === 1 ? "orange" : "#555"}`}
          />
          <h5
            className={`hidden 800px:block pl-2 text-[17px] font-[400] ${
              active === 1 ? "text-[#6eb726]" : "text-[#555]"
            }`}
          >
            Dashboard
          </h5>
        </Link>
      </div>

      <div className='w-full flex items-center p-3'>
        <Link to='/dashboard-orders' className='w-full flex items-center'>
          <FiShoppingBag
            size={28}
            color={`${active === 2 ? "orange" : "#555"}`}
          />
          <h5
            className={`hidden 800px:block pl-2 text-[17px] font-[400] ${
              active === 2 ? "text-[#6eb726]" : "text-[#555]"
            }`}
          >
            All Orders
          </h5>
        </Link>
      </div>

      <div className='w-full flex items-center p-3'>
        <Link to='/dashboard-products' className='w-full flex items-center'>
          <FiPackage size={30} color={`${active === 3 ? "orange" : "#555"}`} />
          <h5
            className={`hidden 800px:block pl-2 text-[17px] font-[400] ${
              active === 3 ? "text-[#6eb726]" : "text-[#555]"
            }`}
          >
            All Products
          </h5>
        </Link>
      </div>

      <div className='w-full flex items-center p-3'>
        <Link
          to='/dashboard-create-product'
          className='w-full flex items-center'
        >
          <AiOutlineFolderAdd
            size={28}
            color={`${active === 4 ? "orange" : "#555"}`}
          />
          <h5
            className={`hidden 800px:block pl-2 text-[17px] font-[400] ${
              active === 4 ? "text-[#6eb726]" : "text-[#555]"
            }`}
          >
            Create Product
          </h5>
        </Link>
      </div>

      <div className='w-full flex items-center p-3'>
        <Link to='/dashboard-events' className='w-full flex items-center'>
          <MdOutlineLocalOffer
            size={28}
            color={`${active === 5 ? "orange" : "#555"}`}
          />
          <h5
            className={`hidden 800px:block pl-2 text-[17px] font-[400] ${
              active === 5 ? "text-[#6eb726]" : "text-[#555]"
            }`}
          >
            All Events
          </h5>
        </Link>
      </div>

      <div className='w-full flex items-center p-3'>
        <Link to='/dashboard-create-event' className='w-full flex items-center'>
          <VscNewFile size={30} color={`${active === 6 ? "orange" : "#555"}`} />
          <h5
            className={`hidden 800px:block pl-2 text-[17px] font-[400] ${
              active === 6 ? "text-[#6eb726]" : "text-[#555]"
            }`}
          >
            Create Event
          </h5>
        </Link>
      </div>

      <div className='w-full flex items-center p-3'>
        <Link
          to='/dashboard-withdraw-money'
          className='w-full flex items-center'
        >
          <CiMoneyBill
            size={28}
            color={`${active === 7 ? "orange" : "#555"}`}
          />
          <h5
            className={`hidden 800px:block pl-2 text-[17px] font-[400] ${
              active === 7 ? "text-[#6eb726]" : "text-[#555]"
            }`}
          >
            Withdraw Money
          </h5>
        </Link>
      </div>

      <div className='w-full flex items-center p-3'>
        <Link to='/dashboard-messages' className='w-full flex items-center'>
          <BiMessageSquareDetail
            size={28}
            color={`${active === 8 ? "orange" : "#555"}`}
          />
          <h5
            className={`hidden 800px:block pl-2 text-[17px] font-[400] ${
              active === 8 ? "text-[#6eb726]" : "text-[#555]"
            }`}
          >
            Shop Inbox
          </h5>
        </Link>
      </div>

      <div className='w-full flex items-center p-3'>
        <Link to='/dashboard-coupouns' className='w-full flex items-center'>
          <AiOutlineGift
            size={28}
            color={`${active === 9 ? "orange" : "#555"}`}
          />
          <h5
            className={`hidden 800px:block pl-2 text-[17px] font-[400] ${
              active === 9 ? "text-[#6eb726]" : "text-[#555]"
            }`}
          >
            Discount Codes
          </h5>
        </Link>
      </div>

      <div className='w-full flex items-center p-3'>
        <Link to='/dashboard-refunds' className='w-full flex items-center'>
          <HiOutlineReceiptRefund
            size={28}
            color={`${active === 10 ? "orange" : "#555"}`}
          />
          <h5
            className={`hidden 800px:block pl-2 text-[17px] font-[400] ${
              active === 10 ? "text-[#6eb726]" : "text-[#555]"
            }`}
          >
            Refunds
          </h5>
        </Link>
      </div>

      <div className='w-full flex items-center p-3'>
        <Link to='/settings' className='w-full flex items-center'>
          <CiSettings
            size={28}
            color={`${active === 11 ? "orange" : "#555"}`}
          />
          <h5
            className={`hidden 800px:block pl-2 text-[17px] font-[400] ${
              active === 11 ? "text-[#6eb726]" : "text-[#555]"
            }`}
          >
            Settings
          </h5>
        </Link>
      </div>
    </div>
  );
};

export default DashboardSideBar;
